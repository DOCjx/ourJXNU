...
jQuery.fn.hyall = function(config){
    ...
    this.formBuilderTypes = {
        ...
        disableedit:function(root, k, v, val){//只读不可编辑
            return '<input type="text" disabled '+(val[k] ? 'value="'+val[k]+'"' : '')+' '+(root.tip ? 'placeholder="'+root.tip+'"' : '')+'" class="form-control '+(root.style || '')+'" '+' name="'+k+'" id="'+k+'" '+(root.attr || '')+' >';
        },
    ...
    this.actionsHandlers = {
        ...
        actionTranslates: function(rows, $modal, columns){
            $.loading();
            $.each(rows,function(k,v){
                // console.log($.U('ajax?q=translates'));
                $.post($.U('ajax?q=translates'), {pk:v}, function(r){
                    // console.log(123);
                    $.unloading();
                    if(!r.status){
                        the.dtActionAlert(r);
                    ...
                    if($.isPlainObject(r.columns)) columns=r.columns;
                    var build=the.formBuilder(columns,r.val,{_submit:'Translates_update',_token:r.token},'translates');
                    //console.log(build);
                    var tpl={
                            modal:'translates',
                            size: $modal.data('size') || '',
                    ...
                    the.setModal(tpl);
                    the.validateFormHandler({form: $modal.find('form'), rules: build.rules, hy: true});
                    if(the.config.options.tips.translates) $modal.find('.alert-tips').show().find('span').html(the.config.options.tips.translates);
                    $modal.on('shown.bs.modal',function(){
                        the.trigger('shown.hyall.form.translates');
                    });
                    $modal.modal('toggle');
                });
            });
    	...
    	actionDisable: function(rows){
    		$.post($.U('ajax?q=disable'), {pk:rows.join(',')},function(r){
    			the.dtActionAlert(r);
    		});
    	},
    	actionEnable: function(rows){
    		$.post($.U('ajax?q=enable'), {pk:rows.join(',')},function(r){
    			the.dtActionAlert(r);
    		});
    	}
    };
