...
abstract class HyAllModel  extends HyFrameModel{
	...
	protected function ajax_translatesAA(&$json){
		// dump(1212);

		$id = act_decrypt($en = I('pk'));
		$GLOBALS['ajaxQuery'] = 'translates';
		$GLOBALS['ajaxPk'] = $id;
		//var_dump($id);
		if($id) $json['val'] = $this->translates($id);
		if($json['status'] = !!$json['val']){
			$json['token'] = token_builder($en);
		}
		$json['info'] = ($json['status'] ? '请求成功！' : ($this->getError() ?: '要请求的数据不可访问！'));
		if($json['status'] && $this->pageOptions['translatesFields'])
			$json['columns'] = $this->packColumns();
	}
	 ...
	 * @param array $json 引用型接收
	 */
	protected function ajax_Translates_update(&$json){
		if(false === ($pk = token_validator(I('_token')))) {
			$log=array('msg'=>"疑似攻击", 'info'=>'text-danger');
			Hook::listen('hy_log', $log);
			$json['info'] = '请勿非法操作，拉黑后果自负！';
		...
		$pk = act_decrypt($pk);
		$json['status'] = !!$this->Translates_update($pk);
		$json['info'] = ($json['status'] ? '信息编辑成功！' : ($this->getError() ?: '信息编辑失败！'));
		$json['reload'] = $this->pageOptions['okRefresh'] ?: false;
	}
	/**
	 ...
	 * @param array $json 引用型接收
	 */
	protected function ajax_disable(&$json){
		if(!is_array($arr=explode(',', (I('pk'))))){
			$json['info']='数据非法！';
			return false;
		}
		...
		$json['status']=!!$this->dis($arr);
	//	$json['sql']=$this->_sql();
		$json['info']=$json['status']?'记录禁用成功！' : ($this->getError() ?: '禁用记录失败！');
	}
	/**
	 * 列表启用数据处理
	 * 
	 ...
	 * @param array $json 引用型接收
	 */
	protected function ajax_enable(&$json){
		if(!is_array($arr=explode(',', (I('pk'))))){
			$json['info']='数据非法！';
			return false;
		}
		...
		$json['status']=!!$this->ena($arr);
	//	$json['sql']=$this->_sql();
		$json['info']=$json['status']?'记录启用成功！' : ($this->getError() ?: '启用记录失败！');
	}
	/**
	 * 列表显示数据响应
	 * 
...
// 		$records['model'] = $this;
	 ...
	 * @param number $pk 要编辑记录的主键
	 */
	protected function translates($pk){
		$error = '无权访问要请求的数据！';
		$field = $this->getPk();
		if(!$field || !$pk) return $this->setError($error);
		// 数据访问检查
		...
		$con = preg_replace("/<(\/p.*?)>/", '', $con);
		$con = strip_tags($con);
		$conArrRaw = explode("=++++++=", $con);
		//将标题加入一起翻译
		array_push($conArrRaw, $title);
		//将每一段向百度翻译请求翻译
		$conArr = array();
		$from = "zh";
		$to = "en";
		$appid = "20171016000088777";//这里填写你在百度上申请的API ID
		$secKey = "nBc0tX1GCq70Bl8vfA2_";//这里填写你在百度上申请的API key
		foreach ($conArrRaw as $v) {
			if($v != '') {
				$v = htmlspecialchars(trim($v));
				$vEncode = urlencode($v);
				$salt = rand(10000,99999);
				$strC = $appid . $v . $salt . $secKey;
		    	$retC = md5($strC);//加密
				$traApi = "http://api.fanyi.baidu.com/api/trans/vip/translate?q=" . $vEncode . "&from=" . $from . "&to=" . $to . "&appid=" . $appid . "&salt=" . $salt . "&sign=" . $retC;
				$textC = json_decode($this->language_text($traApi)); 
				$textC = $textC->trans_result[0]->dst;
				array_push($conArr, $textC);
			}
		}
    	//对段进行拼接,取出标题
    	$value['etitle'] = array_pop($conArr);
		$value['econtent'] = "<p>".implode("</p><p>", $conArr)."</p>";
// var_dump($value);
// 		// $value_content=htmlspecialchars_decode($value['content']);
// 		// $value_title=htmlspecialchars_decode($value['title']);
// 		$value_content=$value['content'];
// 		$value_title=$value['title'];
// // var_dump($value_content);
// 		$value_codeC=urlencode($value['content']);
// 		$value_codeT=urlencode($value['title']);
// 		$appid="20171016000088777";//这里填写你在百度上申请的API ID
// 		$secKey="nBc0tX1GCq70Bl8vfA2_";//这里填写你在百度上申请的API key
// 		$salt= rand(10000,99999);
// 		$strC = $appid . $value_content . $salt . $secKey;
//     	$retC = md5($strC);//加密
//     	$strT = $appid . $value_title . $salt . $secKey;
//     	$retT = md5($strT);//加密
// 		$languageurlC = "http://api.fanyi.baidu.com/api/trans/vip/translate?q=" .$value_codeC. "&from=" . $from . "&to=" . $to . "&appid=" . $appid . "&salt=" . $salt. "&sign=" . $retC ;
// 		$languageurlT = "http://api.fanyi.baidu.com/api/trans/vip/translate?q=" .$value_codeT. "&from=" . $from . "&to=" . $to . "&appid=" . $appid . "&salt=" . $salt. "&sign=" . $retT ;
// 		$textC=json_decode($this->language_text($languageurlC)); 
// 		$textC = $textC->trans_result;
// 		$rsC=$textC[0]->dst;
// 		// $value['econtent']=html_entity_decode($rsC);
// 		$value['econtent']=$rsC;
// 		$textT=json_decode($this->language_text($languageurlT)); 
// 		$textT = $textT->trans_result;
// 		$rsT=$textT[0]->dst;
// 		$value['etitle']=htmlspecialchars($rsT);

		if(!$value) return $this->setError($error);
		// 主键检查
		if(false===$this->checkPk($pk, 'translates')) return $this->setError($error);
		$log=array('msg'=>'数据描述：'.implode('|', $value), 'style'=>'text-info');
		Hook::listen('hy_log', $log);
		// 方法回调支持
		foreach ($this->fieldsOptions as $k=>$v){
			if('file'===$v['form']['type'] && false!==$v['form']['translates'])
				$v['form']['callback']=array('file_down_url');
			if($tmp=$v['form']['callback']){
				$value[$k]=$this->callbackHandler($tmp, $k, $value);
			}
	...
	protected function Translates_update($pk){
		return $this->write('edit', $pk);
	}
	/**
	 * 插入数据处理
	 ...
	 * 禁用数据
	 * 
	 * 支持逻辑禁用（详见HyAllModel:pageOptions->deleteType配置）
	 * @param array $arr 要禁用的主键或者主键数组
	 */
	protected function dis($arr){
		$error = '无权访问要请求的数据！';
		$pk = $this->getPk();
		if(!$pk) return $this->setError($error);
		// 数据访问检查
		...
		if(!$value) return $this->setError($error);
		// 主键检查
		if(false === $this->checkPk($arr,'disable')) return $this->setError($error);
		$log=array('msg'=>'数据描述：'.implode('|', $value), 'style'=>'text-info');
		Hook::listen('hy_log', $log);
		if('disable'==$this->pageOptions['disableType'])
			$data['status']='0';
			return $this->where(array($pk=>array('IN', $arr)))->save($data);
		$dis=explode('|', $this->pageOptions['disableType']);
		if(2 !== count($dis)) return E('disableType配置错误！');
		return $this->where(array($pk=>array('IN', $arr), $dis[0]=>array('lt', $dis[1])))->save(array($dis[0]=>array('exp','(`'.$this->getPk()."`+{$dis[1]})")));
	}
	/**
	 * 启用数据
	 * 
	 * 支持逻辑启用（详见HyAllModel:pageOptions->deleteType配置）
	 * @param array $arr 要启用的主键或者主键数组
	 */
	protected function ena($arr){
		$error = '无权访问要请求的数据！';
		$pk = $this->getPk();
		if(!$pk) return $this->setError($error);
		// 数据访问检查
		...
		if(!$value) return $this->setError($error);
		// 主键检查
		if(false === $this->checkPk($arr,'enable')) return $this->setError($error);
		$log=array('msg'=>'数据描述：'.implode('|', $value), 'style'=>'text-info');
		Hook::listen('hy_log', $log);
		if('enable'==$this->pageOptions['enableType'])
			$data['status']='1';
			return $this->where(array($pk=>array('IN', $arr)))->save($data);
		$dis=explode('|', $this->pageOptions['enableType']);
		if(2 !== count($dis)) return E('enableType配置错误！');
		return $this->where(array($pk=>array('IN', $arr), $dis[0]=>array('lt', $dis[1])))->save(array($dis[0]=>array('exp','(`'.$this->getPk()."`+{$dis[1]})")));
	}
	/**
	 * 方法回调处理
		...
		foreach ($arr as $k=>$v){
			switch($type){
				...
				case 'edit':
					...
					break;
				case 'translates':
					$this->fieldsOptions[$v]['form']['translates'] = false;
					break;
				default:
	...
	protected function callback_unique($val, $key, $data, $other){
		...
		if('disable'!==$this->pageOptions['disableType']){
			$del=explode('|', $this->pageOptions['disableType']);
			$map[$del[0]]=array('neq',$del[1]);
		}
		if('enable'!==$this->pageOptions['enableType']){
			$del=explode('|', $this->pageOptions['enableType']);
			$map[$del[0]]=array('neq',$del[1]);
		}
		if($data[$this->getPk()]){
